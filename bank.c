...
// from-WinHtut-to-lonelone-100
    ...
    int amount_counter = charCounting(change_int_character);
    ...
    if(who == 't'){
        ...
        for(int aaa=0; aaa<amount_counter; aaa++){
            db[transfer].trc[space_array[transfer]-15].note[index_point]=digits[aaa];
            index_point++;
        }
...
void get_time(){
    ...
    printf("Current date and time in Myanmar: %s\n", time_str);
